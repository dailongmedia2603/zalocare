...
interface SourceFormProps {
  ...
  onSubmit: (values: Omit<CustomerSource, 'id'>) => void;
  onClose: () => void;
  isPending: boolean;
}

export const SourceForm = ({ source, onSubmit, onClose, isPending }: SourceFormProps) => {
  const form = useForm<SourceFormValues>({
    resolver: zodResolver(formSchema),
    defaultValues: {
      name: source?.name || "",
      color: source?.color || "bg-blue-500",
    },
  });
  ...
  const handleFormSubmit = (values: SourceFormValues) => {
    // Always use 'Users' as the icon for customer sources
    onSubmit({ name: values.name, color: values.color, icon: 'Users' });
  };

  return (
    <DialogContent>
      <DialogHeader>
        <DialogTitle>{isEditing ? "Chỉnh sửa Nguồn" : "Tạo Nguồn Mới"}</DialogTitle>
        <DialogDescription>
          {isEditing ? "Thay đổi thông tin nguồn khách hàng." : "Tạo một nguồn mới để phân loại khách hàng."}
        </DialogDescription>
      </DialogHeader>
      <Form {...form}>
          ...
          <FormField
            ...
            render={({ field }) => (
              <FormItem>
                ...
                <FormControl>
                  <Input placeholder="Ví dụ: Facebook Ads" {...field} />
                </FormControl>
                <FormMessage />
              </FormItem>
            )}
